...
public class EmployeeService {
    ...
    private void validateBirthDate(Employee employee) {
        ...
        // Replicamos en backend la regla de negocio del frontend: edad mínima de 18 años
        if (employee.getBirthDate() != null
                && employee.getBirthDate().isAfter(LocalDate.now().minusYears(18))) {
            throw new BusinessException("Validación: El empleado debe tener al menos 18 años.");
        }
    }

    private String buildFullName(Employee e) {
        String base = e.getFirstName() + " " + e.getLastName();
        return (e.getSecondLastName() != null && !e.getSecondLastName().isBlank())
                ? base + " " + e.getSecondLastName()
                : base;
    }
